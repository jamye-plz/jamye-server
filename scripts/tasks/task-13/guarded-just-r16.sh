#!/usr/bin/env bash
set -euo pipefail
readonly active_generation='task13-r16-g0-f4-coverage-cli-compat-20260906'
readonly manifest='tests/nix/task-13-assertion-manifest-r16-g0-f4.json'
readonly lock='tests/nix/task-13-assertion-manifest-r16-g0-f4.sha256'
readonly fixed_record='.agents/results/task-13-s2-r16-g0-f4-manifest-digest-20260906.txt'
readonly g0_token='.agents/results/task-4b-redis-publish-completion-g0-f4-r16-20260906.json'
readonly expected_g0_token_sha='c2410a119b6d7ffc48ef419a77e0738c13b990da24056fdb2f31dfe5aa204ae0'
readonly g0_token_review='.agents/results/review-task13-g0-f4-r16-token-integrity-r1-20260906.md'
readonly expected_g0_token_review_sha='9af23809837bab01d30d1005956e31fa39e96816637cefeadcc5820f7de9bdac'
readonly authority_plan='tests/nix/task-13-r16-authority-plan-20260906.json'
readonly expected_authority_plan_sha='9e08402759b24017d2933999df581972a31b7fab9baf2a12c875eb51d99b475c'
readonly authority_requirements='tests/nix/task-13-r16-authority-requirements-20260906.md'
readonly expected_authority_requirements_sha='c78d14ede3d538e82ed9739111755d1d68a23a2790627e5d3c962fac83e9907f'
...
if (($# == 2)) && [[ "$2" == '--receipt' ]]; then
  ...
  printf '%s\n' 'usage: guarded-just-r16.sh task13-r16-g0-f4-coverage-cli-compat-20260906 (--receipt | <recorded-digest> <justfile-relative-path> <recipe>)' >&2
  exit 2
fi
...
[[ "$(pwd -P)" == "$repo_root" ]] || reject 'R16 guard must start at the repository root'
[[ "$generation_id" == "$active_generation" ]] || reject 'unknown Task-13 R16 assertion generation'
...
wait_at_deterministic_post_copy_fixture_barrier() {
  local fixture_mode="${TASK13_R16_POST_COPY_FIXTURE_MODE:-}"
  local fixture_root="${TASK13_R16_FIXTURE_REPO_ROOT:-}"
  local ready_fd="${TASK13_R16_FIXTURE_READY_FD:-}"
  local release_fd="${TASK13_R16_FIXTURE_RELEASE_FD:-}"
  local temp_root release
  if [[ -z "$fixture_mode$fixture_root$ready_fd$release_fd" ]]; then
    return 0
  fi
  [[ "$target_justfile" == 'scripts/tasks/task-13/mod.just' && "$target_recipe" == 'coverage-r16-exec' ]] || reject 'R16 post-copy fixture is restricted to the private coverage GREEN pair'
  [[ "$fixture_mode" == 'content-mutation' || "$fixture_mode" == 'path-replacement' ]] || reject 'R16 post-copy fixture mode is invalid'
  [[ "$fixture_root" == "$repo_root" ]] || reject 'R16 post-copy fixture root does not match the guarded repository'
  temp_root="$(cd -P -- "${TMPDIR:-/tmp}" && pwd -P)" || reject 'R16 post-copy fixture temp root cannot be resolved'
  case "$repo_root" in
    "$temp_root"/jamye-task13-r16-toctou.*/repo) ;;
    *) reject 'R16 post-copy fixture is forbidden outside its disposable repository' ;;
  esac
  [[ "$ready_fd" =~ ^[0-9]+$ && "$release_fd" =~ ^[0-9]+$ && "$ready_fd" != "$release_fd" ]] || reject 'R16 post-copy fixture descriptors are invalid'
  printf 'ready:%s\n' "$fixture_mode" >&"$ready_fd" || reject 'R16 post-copy fixture ready signal failed'
  IFS= read -r release <&"$release_fd" || reject 'R16 post-copy fixture release signal failed'
  [[ "$release" == "release:${fixture_mode}" ]] || reject 'R16 post-copy fixture release token is invalid'
}
for required_path in "$manifest" "$lock" "$fixed_record"; do
  verify_regular_repo_file "$required_path" 'R16 manifest material'
done
verify_manifest_file() {
  local source_file="$1"
  local expected
  cmp -s <(jq -cS '.' "$source_file") "$source_file" || reject 'R16 manifest bytes are not canonical JSON plus LF'
  snapshot_digest="$(sha256sum "$source_file" | awk '{print $1}')"
  [[ "$snapshot_digest" =~ ^[0-9a-f]{64}$ ]] || reject 'R16 manifest digest is malformed'
  expected="${snapshot_digest}"$'\n'
  cmp -s <(printf '%s' "$expected") "$lock" || reject 'R16 manifest lock is malformed or mismatched'
  cmp -s <(printf '%s' "$expected") "$fixed_record" || reject 'R16 coordinator record is malformed or mismatched'
  ...
  ' "$source_file" >/dev/null || reject 'R16 generation or GREEN-only identity is invalid'
}
...
verify_active_authority_prerequisite() {
  verify_regular_repo_file "$g0_token" 'G0-F4 completion token'
  verify_regular_repo_file "$g0_token_review" 'G0-F4 token-integrity review'
  [[ "$(sha256sum "$g0_token" | awk '{print $1}')" == "$expected_g0_token_sha" ]] || reject 'stale G0-F4 completion token'
  [[ "$(sha256sum "$g0_token_review" | awk '{print $1}')" == "$expected_g0_token_review_sha" ]] || reject 'stale G0-F4 token-integrity review'
  [[ "$(rg -Fxc -- '## Review Result: PASS' "$g0_token_review")" == 1 ]] || reject 'G0-F4 token-integrity review is not PASS'
  jq -e \
    ...
    --arg requirements_sha "$expected_authority_requirements_sha" '
      ...
      and .authority_snapshots.runtime_rule == "R16 consumers validate only these immutable snapshot paths and hashes. Mutable canonical plan/requirements and every R11-R15 authority artifact are forbidden runtime inputs and fallbacks."
      and .parent_token.path == ".agents/results/task-4b-redis-publish-completion-g0-f3-r15-20260905.json"
      and .parent_token.sha256 == "ae153d7153c1f2bcc2b613c84d4350d9a1930b006778ccc72c8379aba70556da"
      and .parent_token.schema_version == "1.3"
      and .parent_token.generation_id == "task13-r15-g0-f3-coverage-authority-20260905"
      and .parent_token.status == "complete"
      and .parent_token.integrity_review.sha256 == "dcb59791360fc98d78b69d38995a3acdba145d80b59fb6444ddf182f2a8d514e"
      and .parent_token.integrity_review.status == "PASS"
      and .supersedes.active_predecessor_generation_id == "task13-r15-g0-f3-coverage-authority-20260905"
      and .supersedes.replacement_authority == "R16 immutable plan and requirements snapshots"
      and .supersedes.parent_remains_immutable_historical_evidence == true
      and .supersedes.r11_through_r15_artifacts_remain_immutable_historical_evidence == true
      and .supersedes.coverage_command_argv_superseded == true
      and .supersedes.removed_flag == "--no-clean"
      and .supersedes.affected_ordered_command_indices == [2, 3]
      and .supersedes.only_authority_and_private_coverage_green_executor_binding_superseded == true
      ...
      and .downstream_exclusions.r16_frozen_source_or_guard_bound == false
      and .downstream_exclusions.r16_manifest_lock_or_record_bound == false
      and .downstream_exclusions.final_r16_implementation_ccr_bound == false
    ...
    ' "$g0_token" >/dev/null || reject 'G0-F4 completion token contract is invalid'

  printf '%s\n' 'Task-13 G0-F4/R16 immutable authority token is current'
}
...
verify_authority_snapshots_at_root() {
  ...
  jq -e 'type == "object"' "$plan_file" >/dev/null || reject "R16 authority plan snapshot is invalid JSON at ${phase}"
  [[ "$(tail -c 1 "$plan_file" | od -An -tuC | tr -d '[:space:]')" == 10 ]] || reject "R16 authority plan snapshot lacks final LF at ${phase}"
  [[ "$(tail -c 1 "$requirements_file" | od -An -tuC | tr -d '[:space:]')" == 10 ]] || reject "R16 authority requirements snapshot lacks final LF at ${phase}"
  jq -e \
    ...
    --arg requirements_sha "$expected_authority_requirements_sha" '
      ...
      and .authority_binding.active_assertion_contract == $plan[0].assertion_manifests.task13_r16
      and .authority_binding.active_protocol_authority == $plan[0].active_r16_final_protocol_authority
    ' "$source_file" >/dev/null || reject "R16 immutable authority projection is invalid at ${phase}"
}
...
verify_active_command_source_schema() {
  ...
  jq -e --arg generation_id "$active_generation" '
    ...
    and (.command_source_integrity.canonical_order == [
      "scripts/tasks/task-13/r16-shape.just",
      "scripts/tasks/task-13/guarded-just-r16.sh",
      "tests/nix/flake-shape-aarch64-linux-r16.sh",
      "scripts/tasks/task-13/flake-source-snapshot-r16.sh"
    ])
    ...
    and (all(.command_source_integrity.sources[];
      (.path | test("^(scripts/tasks/task-13/(r16-shape[.]just|guarded-just-r16[.]sh|flake-source-snapshot-r16[.]sh)|tests/nix/flake-shape-aarch64-linux-r16[.]sh)$"))
      and (.sha256 | test("^[0-9a-f]{64}$"))
    ))
    and .command_source_integrity.live_auxiliary_dependencies == []
    and (.command_source_integrity.post_validation_mutation_fixture.id == "active-r16-post-copy-coverage-executor-mutation-zero-nested-dispatch")
    and (.command_source_integrity.post_validation_mutation_fixture.fixture_kind == "executable-production-barrier")
    and (.command_source_integrity.post_validation_mutation_fixture.executable_path == "tests/nix/coverage-executor-toctou-r16.sh")
    and (.command_source_integrity.post_validation_mutation_fixture.executable_sha256 | test("^[0-9a-f]{64}$"))
    and (.command_source_integrity.post_validation_mutation_fixture.public_transport == "just --justfile scripts/tasks/task-13/mod.just coverage-r16 <r16-recorded-digest>")
    ...
    and (.command_source_integrity.post_validation_mutation_fixture.recipe == "coverage-r16-exec")
    and (.command_source_integrity.post_validation_mutation_fixture.coverage_executor_recipes == ["coverage-r16-exec"])
    ...
    and (.guarded_dispatch.execution_classes.immutable_snapshot == [
      {"justfile":"scripts/tasks/task-13/r16-shape.just","recipe":"aarch64-linux-shape-r16-immutable"},
  ...
  ' "$source_file" >/dev/null || reject 'active R16 command-source integrity schema is missing or incompatible'
}
...
verify_active_protocol_schema() {
  ...
  jq -e \
    ...
    --arg review_sha "$expected_g0_token_review_sha" '
      ...
      and .authority_binding.active_assertion_contract.authority_digest == "<r16-recorded-digest>"
      and .authority_binding.active_assertion_contract.coverage.historical_r15_red == {
        evidence_path:".agents/results/task-13-s3-r15-coverage-red-user-evidence-20260906.md",
        sha256:"db4ea8b3b0e750a2488fccb7437f14c7b931f6cd7ae7db425c5b725ba7f63203",
        expected:"exactly 2 FAIL/0 PASS/0 SKIP/non-zero: coverage recipe missing; cargo-llvm-cov devShell tool missing",
        rerun_under_r16:false
      }
      and .authority_binding.active_assertion_contract.coverage.green.ordered_commands == [
        ...
        "CARGO_NET_OFFLINE=true CARGO_LLVM_COV_TARGET_DIR=target/task-13-llvm-cov cargo llvm-cov --locked --offline --workspace --all-targets --no-report",
        "CARGO_NET_OFFLINE=true JAMYE_ENABLE_DEV_FIXTURES=true CARGO_LLVM_COV_TARGET_DIR=target/task-13-llvm-cov cargo llvm-cov --locked --offline --workspace --all-targets --all-features --no-report",
      ...
      and .authority_binding.active_assertion_contract.coverage.fv03_binding == "FV03 retains its fifth public slot exactly as scripts/tasks/task-13/mod.just coverage-r16 <r16-recorded-digest>; that transport selects only coverage-r16-exec and consumes this R16-owned corrected GREEN contract."
      ...
      and ([.guarded_dispatch.allowed_pairs[] | select(.recipe == "coverage-red" or .recipe == "coverage" or .recipe == "coverage-r16")] | length == 0)
      ...
      and .final_tree_record_protocol.descriptor_immutable_file_paths == {
        "aarch64_darwin": {
          "first_receipt": ".agents/results/task-13-r16-aarch64-darwin-first-receipt-20260906.json",
          "fv07_replay_receipt": ".agents/results/task-13-r16-aarch64-darwin-fv07-replay-receipt-20260906.json",
          "terminal": ".agents/results/task-13-r16-aarch64-darwin-terminal-20260906.json"
        },
        "aarch64_linux": {
          "first_receipt": ".agents/results/task-13-r16-aarch64-linux-first-receipt-20260906.json",
          "fv07_replay_receipt": ".agents/results/task-13-r16-aarch64-linux-fv07-replay-receipt-20260906.json",
          "terminal": ".agents/results/task-13-r16-aarch64-linux-terminal-20260906.json"
        },
        "x86_64_linux": {
          "first_receipt": ".agents/results/task-13-r16-x86_64-linux-first-receipt-20260906.json",
          "fv07_replay_receipt": ".agents/results/task-13-r16-x86_64-linux-fv07-replay-receipt-20260906.json",
          "terminal": ".agents/results/task-13-r16-x86_64-linux-terminal-20260906.json"
        }
      }
      ...
      and all(.final_tree_record_protocol.fv_input_sets["FV01", "FV02", "FV03", "FV04", "FV05", "FV06", "FV07", "FV08"];
        ...
        and (.explicit_ignored_evidence_input_ids | index("ignored-r16-recorded-digest")) != null
      )
      and .operator_surfaces.final_verify.dispatch_by_id["fv-07-supported-system-builder-matrix"].dispatch == [
        "scripts/tasks/task-13/mod.just linux-builder-check <r16-recorded-digest> <gpe-map-digest> fv07-replay",
        "scripts/tasks/task-13/mod.just aarch64-linux-builder-check <r16-recorded-digest> <gpe-map-digest> fv07-replay",
        "scripts/tasks/task-13/mod.just cross-system-verify <r16-recorded-digest> fv07-replay"
      ]
      and .operator_surfaces.final_verify.dispatch_by_id["fv-09-redacted-no-mutation-closeout"].handoff_path == ".agents/results/task-13-r16-final-verify-handoff-20260906.json"
      and .final_tree_snapshot.record_protocol_ref == "assertion_manifests.task13_r16.final_tree_record_protocol"
      ...
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f4-authority-plan-snapshot")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f4-authority-requirements-snapshot")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f4-authority-approval")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f4-completion-token")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f4-token-integrity-review")) != null
    ' "$source_file" >/dev/null || reject 'active R16 final-tree, descriptor, or authority schema is missing or incompatible'
}
...
verify_recorded_source_at_root() {
  ...
  [[ "$source_root" == /* && -d "$source_root" ]] || reject "active R16 recorded-source root is invalid at ${phase}"
  [[ "$expected_sha" =~ ^[0-9a-f]{64}$ ]] || reject "active R16 recorded-source digest is malformed at ${phase}"
  case "$relative_path" in
    scripts/tasks/task-13/r16-shape.just | scripts/tasks/task-13/guarded-just-r16.sh | tests/nix/flake-shape-aarch64-linux-r16.sh | scripts/tasks/task-13/flake-source-snapshot-r16.sh | scripts/tasks/task-1/mod.just | tests/nix/task-13-r16-authority-plan-20260906.json | tests/nix/task-13-r16-authority-requirements-20260906.md) ;;
    *) reject "active R16 recorded-source path is noncanonical at ${phase}" ;;
  esac
  source_directory="$source_root/$(dirname -- "$relative_path")"
  source_basename="$(basename -- "$relative_path")"
  [[ -d "$source_directory" ]] || reject "active R16 command-source directory is missing at ${phase}"
  physical_directory="$(cd -P -- "$source_directory" && pwd -P)" || reject "active R16 command-source directory cannot be resolved at ${phase}"
  physical_source="${physical_directory}/${source_basename}"
  [[ "$physical_source" == "$source_root/$relative_path" && -f "$physical_source" && ! -L "$physical_source" ]] || reject "active R16 recorded source is missing, linked, or escaped at ${phase}"
  actual_sha="$(sha256sum "$physical_source" | awk '{print $1}')"
  [[ "$actual_sha" == "$expected_sha" ]] || reject "active R16 recorded-source hash mismatch at ${phase}"
}
...
verify_reused_task1_source_at_root() {
  ...
  local source_file="${3:-$manifest_snapshot}"
  local expected_sha relative_path
  expected_sha="$(jq -er '.command_source_integrity.reused_target_sources[0].sha256' "$source_file")" || reject 'active R16 reused Task-1 source digest is missing'
  relative_path="$(jq -er '.command_source_integrity.reused_target_sources[0].path' "$source_file")" || reject 'active R16 reused Task-1 source path is missing'
  verify_recorded_source_at_root "$source_root" "$phase" "$expected_sha" "$relative_path"
}
...
allowed_pairs_from_snapshot() {
  jq -r '
    ...
    | select(
        ((.justfile == "scripts/tasks/task-13/r16-shape.just") or (.justfile | test("^scripts/tasks/[A-Za-z0-9-]+/mod\\.just$")))
...
if [[ "$mode" == 'receipt' ]]; then
  ...
  printf '%s\n' 'DISPOSITION: ACTIVE_R16_MANIFEST_VERIFIED (zero mutation)'
  exit 0
fi
...
[[ "$target_justfile" == 'scripts/tasks/task-13/r16-shape.just' || "$target_justfile" =~ ^scripts/tasks/[A-Za-z0-9-]+/mod\.just$ ]] || reject 'justfile path is not canonical for active R16'
[[ "$target_recipe" =~ ^[A-Za-z0-9][A-Za-z0-9-]*$ ]] || reject 'recipe is not canonical'
verify_regular_repo_file "$target_justfile" 'R16 target Justfile'
manifest_snapshot="$(mktemp "${TMPDIR:-/tmp}/jamye-task13-r16-manifest.XXXXXX")" || reject 'cannot create R16 manifest snapshot'
cp -- "$manifest" "$manifest_snapshot" || reject 'cannot snapshot R16 manifest'
...
active_execution_class="$(active_execution_class_for_pair)" || reject 'active R16 dispatch pair classification failed'
...
if [[ "$active_execution_class" == 'immutable_snapshot' ]]; then
    ...
    snapshot_helper_relative='scripts/tasks/task-13/flake-source-snapshot-r16.sh'
    snapshot_helper_expected_sha="$(jq -er --arg path "$snapshot_helper_relative" '.command_source_integrity.sources[] | select(.path == $path) | .sha256' "$manifest_snapshot")" || reject 'active R16 snapshot-helper hash is missing'
    snapshot_helper_copy="$(mktemp "${TMPDIR:-/tmp}/jamye-task13-r16-snapshot-helper.XXXXXX")" || reject 'cannot create verified snapshot-helper copy'
...
[[ "$active_execution_class" == 'live_worktree' ]] || reject 'active R16 dispatch pair has an invalid execution class'
...
live_justfile_copy="$(mktemp "$target_directory/.jamye-task13-r16-live-justfile.XXXXXX")" || reject 'cannot create adjacent live target copy'
