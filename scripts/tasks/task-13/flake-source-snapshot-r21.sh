...
if (($# != 0)); then
  printf '%s\n' 'error: flake-source-snapshot-r21.sh accepts no arguments' >&2
  exit 2
fi
...
source_snapshot="$({
  TASK13_SOURCE_ROOT="$repo_root" nix eval --impure --raw --expr '
    let
      ...
      # These are the only generated R21 authority records whose immutable
      # bytes are needed inside the filtered snapshot.  The two parent
      # directories are admitted for traversal only; no sibling leaf and no
      # earlier authority lineage is admitted as a fallback input.
      r21RuntimeAuthorityFiles = [
        ".agents/results/task-4b-redis-publish-completion-g0-f9-r21-20260907.json"
        ".agents/results/review-task13-g0-f9-r21-token-integrity-r1-20260907.md"
        ".agents/results/task-13-s2-r21-g0-f9-manifest-digest-20260907.txt"
      ];
      r21RuntimeAuthorityDirectories = [
        ".agents"
        ".agents/results"
      ];
      relativePath = path:
      ...
      included = path: type:
        let
          ...
          authorityTraversal = type == "directory" && builtins.elem relative r21RuntimeAuthorityDirectories;
        in type != "symlink" && (
          relative == ""
          || builtins.elem relative rootFiles
          || authorityTraversal
          || builtins.elem relative r21RuntimeAuthorityFiles
          || (allowedTree && !(blockedNamedSegment || hiddenSegment || environmentSegment || docsPlans))
        );
    in toString (builtins.path {
      path = root;
      name = "jamye-server-task13-r21-flake-source";
      filter = included;
    })
  '
} )"
