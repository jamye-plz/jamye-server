...
if (($# != 0)); then
  printf '%s\n' 'error: flake-source-snapshot-r20.sh accepts no arguments' >&2
  exit 2
fi
...
source_snapshot="$({
  TASK13_SOURCE_ROOT="$repo_root" nix eval --impure --raw --expr '
    let
      ...
      # These are the only generated R20 authority records whose immutable
      ...
      r20RuntimeAuthorityDirectories = [
        ".agents"
        ".agents/results"
      ];
      relativePath = path:
      ...
      included = path: type:
        let
          ...
          authorityTraversal = type == "directory" && builtins.elem relative r20RuntimeAuthorityDirectories;
        in type != "symlink" && (
          relative == ""
          || builtins.elem relative rootFiles
          || authorityTraversal
          || builtins.elem relative r20RuntimeAuthorityFiles
          || (allowedTree && !(blockedNamedSegment || hiddenSegment || environmentSegment || docsPlans))
        );
    in toString (builtins.path {
      path = root;
      name = "jamye-server-task13-r20-flake-source";
      filter = included;
    })
  '
} )"
