#!/usr/bin/env bash
set -euo pipefail
readonly active_generation='task13-r15-g0-f3-coverage-authority-20260905'
...
readonly fixed_record='.agents/results/task-13-s2-r15-g0-f3-manifest-digest-20260905.txt'
readonly g0_token='.agents/results/task-4b-redis-publish-completion-g0-f3-r15-20260905.json'
readonly expected_g0_token_sha='ae153d7153c1f2bcc2b613c84d4350d9a1930b006778ccc72c8379aba70556da'
readonly g0_token_review='.agents/results/review-task13-g0-f3-r15-token-integrity-r1-20260905.md'
readonly expected_g0_token_review_sha='dcb59791360fc98d78b69d38995a3acdba145d80b59fb6444ddf182f2a8d514e'
readonly authority_plan='tests/nix/task-13-r15-authority-plan-20260905.json'
readonly expected_authority_plan_sha='a818e05c97408b4a038ae96f8db15f5db16a0f6563ac7cf248ca3273cf917f25'
readonly authority_requirements='tests/nix/task-13-r15-authority-requirements-20260905.md'
readonly expected_authority_requirements_sha='948a28ad340f2d2342081702c9d3a012205abbfa67385e1c4ec4fc5ce1d6dd00'
...
if (($# == 2)) && [[ "$2" == '--receipt' ]]; then
  ...
  printf '%s\n' 'usage: guarded-just-r15.sh task13-r15-g0-f3-coverage-authority-20260905 (--receipt | <recorded-digest> <justfile-relative-path> <recipe>)' >&2
  exit 2
fi
...
cleanup() {
  ...
  [[ -z "$coverage_red_assertion_copy" ]] || rm -f -- "$coverage_red_assertion_copy"
}
trap cleanup EXIT
trap 'exit 129' HUP
trap 'exit 130' INT
...
[[ "$(pwd -P)" == "$repo_root" ]] || reject 'R15 guard must start at the repository root'
[[ "$generation_id" == "$active_generation" ]] || reject 'unknown Task-13 R15 assertion generation'
...
wait_at_deterministic_post_copy_fixture_barrier() {
  local fixture_mode="${TASK13_R15_POST_COPY_FIXTURE_MODE:-}"
  local fixture_root="${TASK13_R15_FIXTURE_REPO_ROOT:-}"
  local ready_fd="${TASK13_R15_FIXTURE_READY_FD:-}"
  local release_fd="${TASK13_R15_FIXTURE_RELEASE_FD:-}"
  local temp_root release
  if [[ -z "$fixture_mode$fixture_root$ready_fd$release_fd" ]]; then
    return 0
  fi
  [[ "$target_justfile" == 'scripts/tasks/task-13/mod.just' && "$target_recipe" == 'coverage-red-r15-exec' ]] || reject 'R15 post-copy fixture is restricted to the private coverage RED pair'
  [[ "$fixture_mode" == 'content-mutation' || "$fixture_mode" == 'path-replacement' ]] || reject 'R15 post-copy fixture mode is invalid'
  [[ "$fixture_root" == "$repo_root" ]] || reject 'R15 post-copy fixture root does not match the guarded repository'
  temp_root="$(cd -P -- "${TMPDIR:-/tmp}" && pwd -P)" || reject 'R15 post-copy fixture temp root cannot be resolved'
  case "$repo_root" in
    "$temp_root"/jamye-task13-r15-toctou.*/repo) ;;
    *) reject 'R15 post-copy fixture is forbidden outside its disposable repository' ;;
  esac
  [[ "$ready_fd" =~ ^[0-9]+$ && "$release_fd" =~ ^[0-9]+$ && "$ready_fd" != "$release_fd" ]] || reject 'R15 post-copy fixture descriptors are invalid'
  printf 'ready:%s\n' "$fixture_mode" >&"$ready_fd" || reject 'R15 post-copy fixture ready signal failed'
  IFS= read -r release <&"$release_fd" || reject 'R15 post-copy fixture release signal failed'
  [[ "$release" == "release:${fixture_mode}" ]] || reject 'R15 post-copy fixture release token is invalid'
}
for required_path in "$manifest" "$lock" "$fixed_record"; do
  verify_regular_repo_file "$required_path" 'R15 manifest material'
done
verify_manifest_file() {
  local source_file="$1"
  local expected
  cmp -s <(jq -cS '.' "$source_file") "$source_file" || reject 'R15 manifest bytes are not canonical JSON plus LF'
  snapshot_digest="$(sha256sum "$source_file" | awk '{print $1}')"
  [[ "$snapshot_digest" =~ ^[0-9a-f]{64}$ ]] || reject 'R15 manifest digest is malformed'
  expected="${snapshot_digest}"$'\n'
  cmp -s <(printf '%s' "$expected") "$lock" || reject 'R15 manifest lock is malformed or mismatched'
  cmp -s <(printf '%s' "$expected") "$fixed_record" || reject 'R15 coordinator record is malformed or mismatched'
  ...
  ' "$source_file" >/dev/null || reject 'R15 generation or GREEN-only identity is invalid'
}
...
verify_active_authority_prerequisite() {
  verify_regular_repo_file "$g0_token" 'G0-F3 completion token'
  verify_regular_repo_file "$g0_token_review" 'G0-F3 token-integrity review'
  [[ "$(sha256sum "$g0_token" | awk '{print $1}')" == "$expected_g0_token_sha" ]] || reject 'stale G0-F3 completion token'
  [[ "$(sha256sum "$g0_token_review" | awk '{print $1}')" == "$expected_g0_token_review_sha" ]] || reject 'stale G0-F3 token-integrity review'
  [[ "$(rg -Fxc -- '## Review Result: PASS' "$g0_token_review")" == 1 ]] || reject 'G0-F3 token-integrity review is not PASS'
  jq -e \
    ...
    --arg requirements_sha "$expected_authority_requirements_sha" '
      ...
      and .parent_token.path == ".agents/results/task-4b-redis-publish-completion-g0-f2-r14-20260905.json"
      and .parent_token.schema_version == "1.2"
      and .parent_token.generation_id == "task13-r14-g0-f2-authority-snapshots-20260905"
      and .parent_token.status == "complete"
      and .parent_token.integrity_review.sha256 == "8ff005d0bddff58a95efe77a6b21986cd1cff1d7eb8c065e27f41670af72f1b3"
      and .parent_token.integrity_review.status == "PASS"
      and .supersedes.active_predecessor_generation_id == "task13-r14-g0-f2-authority-snapshots-20260905"
      and .supersedes.replacement_authority == "R15 immutable plan and requirements snapshots"
      and .supersedes.parent_remains_immutable_historical_evidence == true
      and .supersedes.r11_r12_r13_r14_artifacts_remain_immutable_historical_evidence == true
      and .supersedes.only_authority_and_two_private_coverage_executor_bindings_superseded == true
      ...
      and .downstream_exclusions.r15_frozen_source_or_guard_bound == false
      and .downstream_exclusions.r15_manifest_lock_or_record_bound == false
      and .downstream_exclusions.final_r15_implementation_ccr_bound == false
      and .downstream_exclusions.user_receipt_bound == false
      and .downstream_exclusions.coverage_red_or_green_output_bound == false
    ...
    ' "$g0_token" >/dev/null || reject 'G0-F3 completion token contract is invalid'

  printf '%s\n' 'Task-13 G0-F3/R15 immutable authority token is current'
}
...
verify_authority_snapshots_at_root() {
  ...
  jq -e 'type == "object"' "$plan_file" >/dev/null || reject "R15 authority plan snapshot is invalid JSON at ${phase}"
  [[ "$(tail -c 1 "$plan_file" | od -An -tuC | tr -d '[:space:]')" == 10 ]] || reject "R15 authority plan snapshot lacks final LF at ${phase}"
  [[ "$(tail -c 1 "$requirements_file" | od -An -tuC | tr -d '[:space:]')" == 10 ]] || reject "R15 authority requirements snapshot lacks final LF at ${phase}"
  jq -e \
    ...
    --arg requirements_sha "$expected_authority_requirements_sha" '
      ...
      and .authority_binding.active_assertion_contract == $plan[0].assertion_manifests.task13_r15
      and .authority_binding.active_protocol_authority == $plan[0].active_r15_final_protocol_authority
    ' "$source_file" >/dev/null || reject "R15 immutable authority projection is invalid at ${phase}"
}
...
verify_active_command_source_schema() {
  ...
  jq -e --arg generation_id "$active_generation" '
    ...
    and (all(.command_source_integrity.sources[];
      (.path | test("^(scripts/tasks/task-13/(r15-shape[.]just|guarded-just-r15[.]sh|flake-source-snapshot-r15[.]sh)|tests/nix/flake-shape-aarch64-linux-r15[.]sh)$"))
      and (.sha256 | test("^[0-9a-f]{64}$"))
    ))
    and (.command_source_integrity.live_auxiliary_dependencies | length == 1)
    and (.command_source_integrity.live_auxiliary_dependencies[0].id == "coverage-red-assertion")
    and (.command_source_integrity.live_auxiliary_dependencies[0].pair == {justfile:"scripts/tasks/task-13/mod.just", recipe:"coverage-red-r15-exec"})
    and (.command_source_integrity.live_auxiliary_dependencies[0].path == "tests/nix/coverage-red.sh")
    and (.command_source_integrity.live_auxiliary_dependencies[0].sha256 | test("^[0-9a-f]{64}$"))
    and (.command_source_integrity.live_auxiliary_dependencies[0].binding == {
      copy_directory_rule:"same physical directory as live auxiliary source",
      copy_mode:"0400",
      exported_path_variable:"TASK13_COVERAGE_RED_ASSERTION",
      identity_fields:["device", "inode", "sha256"],
      pre_and_post_validation:true,
      zero_live_fallback:true
    })
    and (.command_source_integrity.post_validation_mutation_fixture.id == "active-r15-post-copy-coverage-executor-mutation-zero-nested-dispatch")
    and (.command_source_integrity.post_validation_mutation_fixture.fixture_kind == "executable-production-barrier")
    and (.command_source_integrity.post_validation_mutation_fixture.executable_path == "tests/nix/coverage-executor-toctou-r15.sh")
    and (.command_source_integrity.post_validation_mutation_fixture.executable_sha256 | test("^[0-9a-f]{64}$"))
    and (.command_source_integrity.post_validation_mutation_fixture.public_transport == "just --justfile scripts/tasks/task-13/mod.just coverage-red <r15-recorded-digest>")
    ...
    and (.command_source_integrity.post_validation_mutation_fixture.recipe == "coverage-red-r15-exec")
    and (.command_source_integrity.post_validation_mutation_fixture.coverage_executor_recipes == ["coverage-red-r15-exec", "coverage-r15-exec"])
    ...
    and ([.guarded_dispatch.execution_classes.live_worktree[]
      ...
      | .recipe] | sort == ["coverage-r15-exec", "coverage-red-r15-exec"])
    ...
    and (.guarded_dispatch.execution_classes.immutable_snapshot == [
      {"justfile":"scripts/tasks/task-13/r15-shape.just","recipe":"aarch64-linux-shape-r15-immutable"},
      {"justfile":"scripts/tasks/task-1/mod.just","recipe":"flake-linux"},
      {"justfile":"scripts/tasks/task-1/mod.just","recipe":"flake-local"}
    ])
    and (.guarded_dispatch.execution_classes.live_worktree | length == 17)
    and (.guarded_dispatch.allowed_pairs | length == 20)
  ...
  ' "$source_file" >/dev/null || reject 'active R15 command-source integrity schema is missing or incompatible'
}
...
verify_active_protocol_schema() {
  ...
  jq -e \
    ...
    --arg review_sha "$expected_g0_token_review_sha" '
      ...
      and .authority_binding.active_assertion_contract.authority_digest == "<r15-recorded-digest>"
      and .authority_binding.active_assertion_contract.coverage.red.expected == {
        fail:["coverage recipe missing", "cargo-llvm-cov devShell tool missing"],
        pass:[],
        skip:[],
        exit:"non-zero",
        exact_summary:"exactly 2 FAIL/0 PASS/0 SKIP/non-zero"
      }
      and .authority_binding.active_assertion_contract.coverage.green.ordered_commands == [
        "CARGO_LLVM_COV_TARGET_DIR=target/task-13-llvm-cov cargo llvm-cov clean --workspace",
        "CARGO_NET_OFFLINE=true CARGO_LLVM_COV_TARGET_DIR=target/task-13-llvm-cov cargo llvm-cov --locked --offline --workspace --all-targets --no-clean --no-report",
        "CARGO_NET_OFFLINE=true JAMYE_ENABLE_DEV_FIXTURES=true CARGO_LLVM_COV_TARGET_DIR=target/task-13-llvm-cov cargo llvm-cov --locked --offline --workspace --all-targets --all-features --no-clean --no-report",
      ...
      and .authority_binding.active_assertion_contract.coverage.fv03_binding == "FV03 retains its fifth public slot exactly as scripts/tasks/task-13/mod.just coverage <r15-recorded-digest>; that transport selects only coverage-r15-exec and consumes this R15-owned green contract."
      ...
      and ([.guarded_dispatch.allowed_pairs[] | select(.recipe == "coverage-red" or .recipe == "coverage")] | length == 0)
      ...
      and .final_tree_record_protocol.descriptor_immutable_file_paths == {
        "aarch64_darwin": {
          "first_receipt": ".agents/results/task-13-r15-aarch64-darwin-first-receipt-20260905.json",
          "fv07_replay_receipt": ".agents/results/task-13-r15-aarch64-darwin-fv07-replay-receipt-20260905.json",
          "terminal": ".agents/results/task-13-r15-aarch64-darwin-terminal-20260905.json"
        },
        "aarch64_linux": {
          "first_receipt": ".agents/results/task-13-r15-aarch64-linux-first-receipt-20260905.json",
          "fv07_replay_receipt": ".agents/results/task-13-r15-aarch64-linux-fv07-replay-receipt-20260905.json",
          "terminal": ".agents/results/task-13-r15-aarch64-linux-terminal-20260905.json"
        },
        "x86_64_linux": {
          "first_receipt": ".agents/results/task-13-r15-x86_64-linux-first-receipt-20260905.json",
          "fv07_replay_receipt": ".agents/results/task-13-r15-x86_64-linux-fv07-replay-receipt-20260905.json",
          "terminal": ".agents/results/task-13-r15-x86_64-linux-terminal-20260905.json"
        }
      }
      ...
      and all(.final_tree_record_protocol.fv_input_sets["FV01", "FV02", "FV03", "FV04", "FV05", "FV06", "FV07", "FV08"];
        ...
        and (.explicit_ignored_evidence_input_ids | index("ignored-r15-recorded-digest")) != null
      )
      and .operator_surfaces.final_verify.dispatch_by_id["fv-07-supported-system-builder-matrix"].dispatch == [
        "scripts/tasks/task-13/mod.just linux-builder-check <r15-recorded-digest> <gpe-map-digest> fv07-replay",
        "scripts/tasks/task-13/mod.just aarch64-linux-builder-check <r15-recorded-digest> <gpe-map-digest> fv07-replay",
        "scripts/tasks/task-13/mod.just cross-system-verify <r15-recorded-digest> fv07-replay"
      ]
      and .operator_surfaces.final_verify.dispatch_by_id["fv-09-redacted-no-mutation-closeout"].handoff_path == ".agents/results/task-13-r15-final-verify-handoff-20260905.json"
      and .final_tree_snapshot.record_protocol_ref == "assertion_manifests.task13_r15.final_tree_record_protocol"
      ...
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f3-authority-plan-snapshot")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f3-authority-requirements-snapshot")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f3-authority-approval")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f3-completion-token")) != null
      and ([.final_tree_snapshot.catalog_required_entries[]] | index("g0-f3-token-integrity-review")) != null
    ' "$source_file" >/dev/null || reject 'active R15 final-tree, descriptor, or authority schema is missing or incompatible'
}
...
verify_recorded_source_at_root() {
  ...
  [[ "$source_root" == /* && -d "$source_root" ]] || reject "active R15 recorded-source root is invalid at ${phase}"
  [[ "$expected_sha" =~ ^[0-9a-f]{64}$ ]] || reject "active R15 recorded-source digest is malformed at ${phase}"
  case "$relative_path" in
    scripts/tasks/task-13/r15-shape.just | scripts/tasks/task-13/guarded-just-r15.sh | tests/nix/flake-shape-aarch64-linux-r15.sh | scripts/tasks/task-13/flake-source-snapshot-r15.sh | scripts/tasks/task-1/mod.just | tests/nix/task-13-r15-authority-plan-20260905.json | tests/nix/task-13-r15-authority-requirements-20260905.md) ;;
    *) reject "active R15 recorded-source path is noncanonical at ${phase}" ;;
  esac
  source_directory="$source_root/$(dirname -- "$relative_path")"
  source_basename="$(basename -- "$relative_path")"
  [[ -d "$source_directory" ]] || reject "active R15 command-source directory is missing at ${phase}"
  physical_directory="$(cd -P -- "$source_directory" && pwd -P)" || reject "active R15 command-source directory cannot be resolved at ${phase}"
  physical_source="${physical_directory}/${source_basename}"
  [[ "$physical_source" == "$source_root/$relative_path" && -f "$physical_source" && ! -L "$physical_source" ]] || reject "active R15 recorded source is missing, linked, or escaped at ${phase}"
  actual_sha="$(sha256sum "$physical_source" | awk '{print $1}')"
  [[ "$actual_sha" == "$expected_sha" ]] || reject "active R15 recorded-source hash mismatch at ${phase}"
}
...
verify_reused_task1_source_at_root() {
  ...
  local source_file="${3:-$manifest_snapshot}"
  local expected_sha relative_path
  expected_sha="$(jq -er '.command_source_integrity.reused_target_sources[0].sha256' "$source_file")" || reject 'active R15 reused Task-1 source digest is missing'
  relative_path="$(jq -er '.command_source_integrity.reused_target_sources[0].path' "$source_file")" || reject 'active R15 reused Task-1 source path is missing'
  verify_recorded_source_at_root "$source_root" "$phase" "$expected_sha" "$relative_path"
}
...
allowed_pairs_from_snapshot() {
  jq -r '
    ...
    | select(
        ((.justfile == "scripts/tasks/task-13/r15-shape.just") or (.justfile | test("^scripts/tasks/[A-Za-z0-9-]+/mod\\.just$")))
...
if [[ "$mode" == 'receipt' ]]; then
  ...
  printf '%s\n' 'DISPOSITION: ACTIVE_R15_MANIFEST_VERIFIED (zero mutation)'
  exit 0
fi
...
[[ "$target_justfile" == 'scripts/tasks/task-13/r15-shape.just' || "$target_justfile" =~ ^scripts/tasks/[A-Za-z0-9-]+/mod\.just$ ]] || reject 'justfile path is not canonical for active R15'
[[ "$target_recipe" =~ ^[A-Za-z0-9][A-Za-z0-9-]*$ ]] || reject 'recipe is not canonical'
verify_regular_repo_file "$target_justfile" 'R15 target Justfile'
manifest_snapshot="$(mktemp "${TMPDIR:-/tmp}/jamye-task13-r15-manifest.XXXXXX")" || reject 'cannot create R15 manifest snapshot'
cp -- "$manifest" "$manifest_snapshot" || reject 'cannot snapshot R15 manifest'
...
active_execution_class="$(active_execution_class_for_pair)" || reject 'active R15 dispatch pair classification failed'
...
if [[ "$active_execution_class" == 'immutable_snapshot' ]]; then
    ...
    snapshot_helper_relative='scripts/tasks/task-13/flake-source-snapshot-r15.sh'
    snapshot_helper_expected_sha="$(jq -er --arg path "$snapshot_helper_relative" '.command_source_integrity.sources[] | select(.path == $path) | .sha256' "$manifest_snapshot")" || reject 'active R15 snapshot-helper hash is missing'
    snapshot_helper_copy="$(mktemp "${TMPDIR:-/tmp}/jamye-task13-r15-snapshot-helper.XXXXXX")" || reject 'cannot create verified snapshot-helper copy'
...
[[ "$active_execution_class" == 'live_worktree' ]] || reject 'active R15 dispatch pair has an invalid execution class'
...
live_justfile_copy="$(mktemp "$target_directory/.jamye-task13-r15-live-justfile.XXXXXX")" || reject 'cannot create adjacent live target copy'
...
coverage_red_assertion_relative=''
if [[ "$target_justfile" == 'scripts/tasks/task-13/mod.just' && "$target_recipe" == 'coverage-red-r15-exec' ]]; then
  coverage_red_assertion_relative="$(jq -er '.command_source_integrity.live_auxiliary_dependencies[0].path' "$manifest_snapshot")" || reject 'coverage RED auxiliary path is missing'
  coverage_red_assertion_expected_sha="$(jq -er '.command_source_integrity.live_auxiliary_dependencies[0].sha256' "$manifest_snapshot")" || reject 'coverage RED auxiliary digest is missing'
  verify_regular_repo_file "$coverage_red_assertion_relative" 'coverage-red-assertion-before-copy'
  coverage_red_assertion_directory="$repo_root/$(dirname -- "$coverage_red_assertion_relative")"
  coverage_red_assertion_basename="$(basename -- "$coverage_red_assertion_relative")"
  coverage_red_assertion_physical_directory="$(cd -P -- "$coverage_red_assertion_directory" && pwd -P)" || reject 'coverage RED assertion directory cannot be resolved'
  coverage_red_assertion_physical="${coverage_red_assertion_physical_directory}/${coverage_red_assertion_basename}"
  coverage_red_assertion_identity_before="$(stat -c '%d:%i' -- "$coverage_red_assertion_physical")" || reject 'coverage RED assertion identity cannot be read before copy'
  [[ "$(sha256sum "$coverage_red_assertion_physical" | awk '{print $1}')" == "$coverage_red_assertion_expected_sha" ]] || reject 'coverage RED assertion digest differs from the verified manifest'
  coverage_red_assertion_copy="$(mktemp "$coverage_red_assertion_physical_directory/.jamye-task13-r15-coverage-red.XXXXXX")" || reject 'cannot create adjacent coverage RED assertion copy'
  cp -- "$coverage_red_assertion_physical" "$coverage_red_assertion_copy" || reject 'cannot copy coverage RED assertion'
  chmod 0400 -- "$coverage_red_assertion_copy" || reject 'cannot make coverage RED assertion copy read-only'
  coverage_red_assertion_copy="${coverage_red_assertion_physical_directory}/$(basename -- "$coverage_red_assertion_copy")"
  coverage_red_assertion_copy_identity="$(stat -c '%d:%i' -- "$coverage_red_assertion_copy")" || reject 'coverage RED assertion copy identity cannot be read'
  [[ "$(sha256sum "$coverage_red_assertion_copy" | awk '{print $1}')" == "$coverage_red_assertion_expected_sha" ]] || reject 'coverage RED assertion copy differs from selected bytes'
fi
...
if [[ -n "$coverage_red_assertion_relative" ]]; then
  verify_bound_live_file_and_copy "$coverage_red_assertion_relative" "$coverage_red_assertion_physical" "$coverage_red_assertion_identity_before" "$coverage_red_assertion_expected_sha" "$coverage_red_assertion_copy" "$coverage_red_assertion_copy_identity" 'coverage-red-assertion-final-pre-dispatch'
fi
printf 'task13_assertion_manifest_sha256=%s\n' "$snapshot_digest"
printf 'task13_assertion_generation=%s\n' "$generation_id"
...
if [[ -n "$coverage_red_assertion_relative" ]]; then
  export TASK13_COVERAGE_RED_ASSERTION="$coverage_red_assertion_copy"
  export TASK13_COVERAGE_RED_ASSERTION_IDENTITY="$coverage_red_assertion_copy_identity"
  export TASK13_COVERAGE_RED_ASSERTION_SHA256="$coverage_red_assertion_expected_sha"
else
...
rm -f -- "$live_justfile_copy" ${coverage_red_assertion_copy:+"$coverage_red_assertion_copy"}
live_justfile_copy=''
coverage_red_assertion_copy=''
exit "$nested_exit"
