...
coordination_dir="$(mktemp -d "${TMPDIR:-/tmp}/jamye-task-5-redis.XXXXXX")"
test_pid=''
redis_stopped=0
...
export JAMYE_TASK5_RECOVERY_COORD_DIR="$coordination_dir"
cargo test --locked --test rate_limit \
  redis_stop_restart_recovers_same_limiter_without_touching_refresh_authority -- \
...
wait "$test_pid"
test_pid=''
printf '%s\n' 'same Redis limiter recovered; PostgreSQL refresh-session authority was unchanged'
