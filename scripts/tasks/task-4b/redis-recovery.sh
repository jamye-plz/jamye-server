...
coordination_dir="$(mktemp -d "${TMPDIR:-/tmp}/jamye-task-4b-redis.XXXXXX")"
test_pid=''
redis_stopped=0
...
export JAMYE_TASK4B_RECOVERY_COORD_DIR="$coordination_dir"
cargo test --locked --test realtime \
  'redis_recovery::redis_stop_restart_keeps_postgres_correctness_and_same_router_recovery' -- \
...
wait "$test_pid"
test_pid=''
printf '%s\n' 'same-router Redis stop/start recovery passed; PostgreSQL outbox bytes were preserved'
