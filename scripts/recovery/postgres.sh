...
if ! compose exec -T postgres pg_isready -U jamye_test -d jamye_test >/dev/null 2>&1; then
  printf '%s\n' 'error: guarded local PostgreSQL is not healthy; run `just infra-up` first' >&2
  exit 2
fi
...
coordination_dir="$(mktemp -d "${TMPDIR:-/tmp}/jamye-postgres-recovery.XXXXXX")"
test_pid=''
postgres_stopped=0
...
cleanup() {
  ...
  if [[ "$postgres_stopped" -eq 1 ]]; then
    printf '%s\n' 'restoring guarded local PostgreSQL after interrupted recovery test' >&2
...
export JAMYE_POSTGRES_RECOVERY_COORD_DIR="$coordination_dir"
JAMYE_ENABLE_DEV_FIXTURES=true \
