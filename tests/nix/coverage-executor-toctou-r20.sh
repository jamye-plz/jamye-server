...
readonly record='.agents/results/task-13-s2-r20-g0-f8-manifest-digest-20260907.txt'
...
cleanup() {
  ...
  for disposable_root in "${disposable_roots[@]}"; do
    case "$disposable_root" in
      "$temp_root"/jamye-task13-r20-toctou.*) rm -rf -- "$disposable_root" ;;
...
[[ -f "$manifest" && ! -L "$manifest" ]] || fail 'R20 manifest is missing or linked'
cmp -s <(jq -cS '.' "$manifest") "$manifest" || fail 'R20 manifest is not canonical JSON plus LF'
manifest_digest="$(sha256sum "$manifest" | awk '{print $1}')"
cmp -s <(printf '%s\n' "$manifest_digest") "$lock" || fail 'R20 manifest lock mismatch'
cmp -s <(printf '%s\n' "$manifest_digest") "$record" || fail 'R20 coordinator record mismatch'
expected_fixture_sha="$(jq -er '.command_source_integrity.post_validation_mutation_fixture.executable_sha256' "$manifest")" || fail 'fixture digest missing from R20 manifest'
[[ "$(sha256sum "$script_path" | awk '{print $1}')" == "$expected_fixture_sha" ]] || fail 'fixture bytes differ from R20 manifest'
...
copy_fixture_repository() {
  ...
  local -a required_paths=(
    ...
    tests/nix/task-13-assertion-manifest-r20-g0-f8.sha256
    tests/nix/task-13-r20-authority-plan-20260907.json
    tests/nix/task-13-r20-authority-requirements-20260907.md
    ...
    .agents/results/task-13-s2-r20-g0-f8-manifest-digest-20260907.txt
  )
...
run_mutation_case() {
  ...
  disposable_root="$(mktemp -d "$temp_root/jamye-task13-r20-toctou.XXXXXX")" || fail 'cannot create disposable fixture root'
  ...
  mkdir -p -- "$fake_bin"
  printf '%s\n' '#!/usr/bin/env bash' 'set -euo pipefail' ': > "${TASK13_R20_NESTED_SENTINEL:?}"' 'exit 0' >"$fake_bin/just"
  ...
  (
    cd "$sandbox"
    PATH="$fake_bin:$PATH" \
    TASK13_R20_NESTED_SENTINEL="$sentinel" \
    TASK13_R20_POST_COPY_FIXTURE_MODE="$fixture_mode" \
    TASK13_R20_FIXTURE_REPO_ROOT="$sandbox" \
    TASK13_R20_FIXTURE_READY_FD="$ready_fd" \
    TASK13_R20_FIXTURE_RELEASE_FD="$release_fd" \
      "$real_just" --justfile scripts/tasks/task-13/mod.just coverage-r20 "$manifest_digest"
  ) >"$output_file" 2>&1 &
  child_pid=$!
  ...
  printf 'PASS: R20 coverage GREEN executor %s exits 2 with zero nested dispatch\n' "$fixture_mode"
}
