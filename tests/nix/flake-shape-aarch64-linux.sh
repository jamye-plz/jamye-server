...
# Active R12 shape contract. Top-level system-key probes are non-forcing; a
# missing aarch64-linux key is never followed by a child probe for that key.
readonly generation_id='task13-r12-aarch64-linux-20260905'
...
readonly missing_system='aarch64-linux'
...
is_exact_missing_system() {
  local observed="$1"
  [[ "$observed" == "$expected_without_missing" ]]
}

declare -a failure_names=()

source_snapshot="${TASK13_FLAKE_SOURCE_SNAPSHOT:-}"
[[ "${TASK13_ASSERTION_GENERATION:-}" == "$generation_id" ]] || invalid 'guarded active R12 generation receipt missing'
[[ "${TASK13_ASSERTION_DIGEST:-}" =~ ^[0-9a-f]{64}$ ]] || invalid 'guarded active R12 digest receipt missing'
...
[[ -f "$manifest" ]] || invalid 'active R12 assertion manifest missing'
cmp -s <(jq -cS '.' "$manifest") "$manifest" || invalid 'active R12 assertion manifest bytes are not canonical JSON plus LF'
[[ "$(sha256sum "$manifest" | awk '{print $1}')" == "$TASK13_ASSERTION_DIGEST" ]] || invalid 'snapshot R12 assertion manifest does not match the guarded digest receipt'
jq -e --arg generation_id "$generation_id" --arg missing_system "$missing_system" '
  ...
  and (.flake_shape.baseline_red_failures == ["supported-system matrix missing aarch64-linux"])
  and (.flake_shape.missing_system_red.system == $missing_system)
  and (.flake_shape.missing_system_red.nested_detail_order == ["packages", "devShells", "checks", "supportedSystems"])
  and (.flake_shape.per_system_family_system_key_sets.families == ["packages", "devShells", "checks"])
' "$manifest" >/dev/null || invalid 'active R12 assertion manifest schema is incompatible'
...
expected_without_missing="$(printf '%s\n' "${expected_system_array[@]}" | sed "/^${missing_system}$/d" | paste -sd, -)"
expected_checks="$(jq -er '.flake_shape.checks_key_set | sort | join(",")' "$manifest")"
[[ "$expected_systems" == 'aarch64-darwin,aarch64-linux,x86_64-linux' ]] || invalid 'active R12 expected systems are incompatible'
[[ "$expected_without_missing" == 'aarch64-darwin,x86_64-linux' ]] || invalid 'active R12 missing-system baseline is incompatible'
[[ "$expected_checks" == 'api,architecture,cargo-clippy,cargo-fmt,cargo-test-all-features,cargo-test-default,contract-drift,worker' ]] || invalid 'active R12 checks inventory is incompatible'
[[ "${baseline_failures[0]}" == 'supported-system matrix missing aarch64-linux' && "${#baseline_failures[@]}" == 1 ]] || invalid 'active R12 baseline failure inventory is incompatible'
...
matrix_is_required_red=true
for family in "${expected_family_array[@]}"; do
  is_exact_missing_system "${observed_family_systems[$family]}" || matrix_is_required_red=false
done
is_exact_missing_system "$supported_systems" || matrix_is_required_red=false

if "$matrix_exact"; then
  pass 'supported-system matrix exact' "${family_system_details[*]} supportedSystems=${supported_systems}"
elif "$matrix_is_required_red"; then
  fail "${baseline_failures[0]}" "$matrix_detail"
  pass 'missing aarch64-linux child probes were not forced'
else
...
if ((${#failure_names[@]} == 1)) && [[ "${failure_names[0]}" == "${baseline_failures[0]}" && "$matrix_is_required_red" == true ]]; then
  printf 'DISPOSITION: BASELINE_RED (one named failure)\n'
  exit 1
fi
