...
# Active R13 GREEN-only shape contract. Top-level system-key probes are
# non-forcing; any missing or extra system/key is invalid acceptance evidence.
readonly generation_id='task13-r13-g0-f1-format-drift-20260905'
...
[[ "${TASK13_ASSERTION_GENERATION:-}" == "$generation_id" ]] || invalid 'guarded active R13 generation receipt missing'
[[ "${TASK13_ASSERTION_DIGEST:-}" =~ ^[0-9a-f]{64}$ ]] || invalid 'guarded active R13 digest receipt missing'
...
[[ -f "$manifest" ]] || invalid 'active R13 assertion manifest missing'
cmp -s <(jq -cS '.' "$manifest") "$manifest" || invalid 'active R13 assertion manifest bytes are not canonical JSON plus LF'
[[ "$(sha256sum "$manifest" | awk '{print $1}')" == "$TASK13_ASSERTION_DIGEST" ]] || invalid 'snapshot R13 assertion manifest does not match the guarded digest receipt'
...
' "$manifest" >/dev/null || invalid 'active R13 assertion manifest schema is incompatible'
...
[[ "$expected_systems" == 'aarch64-darwin,aarch64-linux,x86_64-linux' ]] || invalid 'active R13 expected systems are incompatible'
[[ "$expected_checks" == 'api,architecture,cargo-clippy,cargo-fmt,cargo-test-all-features,cargo-test-default,contract-drift,worker' ]] || invalid 'active R13 checks inventory is incompatible'
...
invalid "R13 GREEN-only assertion mismatch: ${failure_names[*]}"
