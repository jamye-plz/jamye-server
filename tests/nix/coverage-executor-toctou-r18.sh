...
readonly generation_id='task13-r18-g0-f6-review-binding-correction-20260906'
...
readonly record='.agents/results/task-13-s2-r18-g0-f6-manifest-digest-20260906.txt'
...
cleanup() {
  ...
  for disposable_root in "${disposable_roots[@]}"; do
    case "$disposable_root" in
      "$temp_root"/jamye-task13-r18-toctou.*) rm -rf -- "$disposable_root" ;;
...
[[ -f "$manifest" && ! -L "$manifest" ]] || fail 'R18 manifest is missing or linked'
cmp -s <(jq -cS '.' "$manifest") "$manifest" || fail 'R18 manifest is not canonical JSON plus LF'
manifest_digest="$(sha256sum "$manifest" | awk '{print $1}')"
cmp -s <(printf '%s\n' "$manifest_digest") "$lock" || fail 'R18 manifest lock mismatch'
cmp -s <(printf '%s\n' "$manifest_digest") "$record" || fail 'R18 coordinator record mismatch'
expected_fixture_sha="$(jq -er '.command_source_integrity.post_validation_mutation_fixture.executable_sha256' "$manifest")" || fail 'fixture digest missing from R18 manifest'
[[ "$(sha256sum "$script_path" | awk '{print $1}')" == "$expected_fixture_sha" ]] || fail 'fixture bytes differ from R18 manifest'
...
copy_fixture_repository() {
  ...
  local -a required_paths=(
    ...
    tests/nix/task-13-assertion-manifest-r18-g0-f6.json
...
run_mutation_case() {
  ...
  disposable_root="$(mktemp -d "$temp_root/jamye-task13-r18-toctou.XXXXXX")" || fail 'cannot create disposable fixture root'
  ...
  mkdir -p -- "$fake_bin"
  printf '%s\n' '#!/usr/bin/env bash' 'set -euo pipefail' ': > "${TASK13_R18_NESTED_SENTINEL:?}"' 'exit 0' >"$fake_bin/just"
  ...
  (
    cd "$sandbox"
    PATH="$fake_bin:$PATH" \
    TASK13_R18_NESTED_SENTINEL="$sentinel" \
    TASK13_R18_POST_COPY_FIXTURE_MODE="$fixture_mode" \
    TASK13_R18_FIXTURE_REPO_ROOT="$sandbox" \
    TASK13_R18_FIXTURE_READY_FD="$ready_fd" \
    TASK13_R18_FIXTURE_RELEASE_FD="$release_fd" \
      "$real_just" --justfile scripts/tasks/task-13/mod.just coverage-r18 "$manifest_digest"
  ) >"$output_file" 2>&1 &
  child_pid=$!
  ...
  printf 'PASS: R18 coverage GREEN executor %s exits 2 with zero nested dispatch\n' "$fixture_mode"
}
