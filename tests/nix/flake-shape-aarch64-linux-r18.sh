...
# Active R18 GREEN-only shape contract. Top-level system-key probes are
...
[[ "${TASK13_ASSERTION_GENERATION:-}" == "$generation_id" ]] || invalid 'guarded active R18 generation receipt missing'
[[ "${TASK13_ASSERTION_DIGEST:-}" =~ ^[0-9a-f]{64}$ ]] || invalid 'guarded active R18 digest receipt missing'
...
[[ -f "$manifest" ]] || invalid 'active R18 assertion manifest missing'
cmp -s <(jq -cS '.' "$manifest") "$manifest" || invalid 'active R18 assertion manifest bytes are not canonical JSON plus LF'
[[ "$(sha256sum "$manifest" | awk '{print $1}')" == "$TASK13_ASSERTION_DIGEST" ]] || invalid 'snapshot R18 assertion manifest does not match the guarded digest receipt'
...
' "$manifest" >/dev/null || invalid 'active R18 assertion manifest schema is incompatible'
...
[[ "$expected_systems" == 'aarch64-darwin,aarch64-linux,x86_64-linux' ]] || invalid 'active R18 expected systems are incompatible'
[[ "$expected_checks" == 'api,architecture,cargo-clippy,cargo-fmt,cargo-test-all-features,cargo-test-default,contract-drift,worker' ]] || invalid 'active R18 checks inventory is incompatible'
...
invalid "R18 GREEN-only assertion mismatch: ${failure_names[*]}"
