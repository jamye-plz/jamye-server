...
# Task-13 R21 strict successor requirements

## Status and sole purpose

R21 is a candidate-only, append-only authority successor. Its generation is
`task13-r21-g0-f9-frozen-coverage-entrypoint-20260907`. It corrects the one
HIGH integrity defect confirmed only after the R20 manifest was issued: the
R20 coverage executor is selected from mutable `scripts/tasks/task-13/mod.just`
and its expected digest is derived from those same mutable bytes. The adjacent
copy barrier detects post-selection mutation but cannot detect a persistent
pre-selection replacement. The same mutable file is also not a strict
acceptance bootstrap because its public wrapper can be changed to avoid the
guard entirely.

R21 changes only this source-integrity and bootstrap boundary. It does not
change a coverage command, target, test, profile, feature set, scheduling rule,
toolchain or Nix pin, threshold, product/API/schema/migration behavior,
deployment contract, system matrix, task identity, or FV identity/order/count.

## Immutable blocked R20 provenance

The following R20 artifacts are byte-preserved issuance provenance only:

- R20 token:
  `.agents/results/task-4b-redis-publish-completion-g0-f8-r20-20260907.json`
  (`d4cad5b20b5461abee0907431f67158c1175832f6618a49f92786e2c148d52ac`);
- R20 sole post-token review:
  `.agents/results/review-task13-g0-f8-r20-token-integrity-r1-20260907.md`
  (`06e5d0f442b8b59413c96c21d6b1c39130a92dec44d5c80092477bf8599fae74`);
- R20 manifest:
  `tests/nix/task-13-assertion-manifest-r20-g0-f8.json`
  (`a735b76263315f83368c839e968530ce65e26715643719859651e8b080aa6ec6`),
  whose lock and coordinator record each contain that digest plus one LF;
- R20 implementation alignment PASS review:
  `.agents/results/review-task13-r20-implementation-alignment-r1-20260907.md`
  (`db801a27dd600e49413432f86ece6a545c64a61d0af8a6fbf517ad7454595a19`);
- R20 implementation safety FAIL review:
  `.agents/results/review-task13-r20-implementation-safety-r1-20260907.md`
  (`d6745188c6705a24e0e6326cf2dc327966a3ba302d30dc92e3d1103ec716b6fc`);
- confirmed defect:
  `.agents/results/bugs/bug-20260907-r20-mutable-coverage-executor-trust-root.md`
  (`c3a168dd3df831a01ea6a5e7290b6aaf5902d32f33f0d4269cd0f74673abc529`).

No R20 implementation simplicity review exists or may be invented. No R20
receipt, shape, guarded-flake, coverage, module, builder, cross-system, FV, or
handoff result may be created or interpreted as GREEN authority. R20 runtime
files, manifest, lock, record, reviews, and defect remain historical inputs to
R21 issuance only; R21 runtime must never open, hash, compare, dispatch, or
fall back to any R11-R20 authority artifact.

## Frozen executor and exact pair model

`scripts/tasks/task-13/r21-shape.just` is a single manifest-bound frozen
Justfile containing both `aarch64-linux-shape-r21-immutable` and the private
`coverage-r21-exec`. The coverage executor body is copied from R20 without any
coverage-command or behavior change except R21 generation/path/receipt names.
It executes with the live repository as its working directory; it never runs
coverage in the read-only filtered Nix source snapshot.

R21 retains exactly twenty-four unique pairs split into exactly three
`immutable_snapshot` and twenty-one `live_worktree` pairs. Here
`live_worktree` describes the execution root, not whether the selected recipe
source is mutable. The coverage pair is exactly
`scripts/tasks/task-13/r21-shape.just::coverage-r21-exec`, carries
`source_binding=manifest_bound_frozen_source` and
`execution_root=live_repository`, and is the only Task-13 coverage executor.
The other fifteen inherited live-worktree pairs retain their prior delegated
behavior. Five additional manifest-bound private control pairs in the same
frozen Justfile are exact: `module-eval-r21-exec`,
`linux-builder-r21-exec`, `aarch64-linux-builder-r21-exec`,
`cross-system-verify-r21-exec`, and `final-verify-r21-exec`. Every R11-R20
private executor, direct helper entrypoint, and public transport is excluded.

For the coverage pair, the R21 guard must obtain the expected
`r21-shape.just` digest from `command_source_integrity.sources`, not from the
selected live file. Before copying, after copying, and immediately before
nested execution it requires the live source and read-only adjacent copy to
match that manifest-bound digest and their recorded physical identities. Any
missing source, symlink, path escape, persistent pre-selection modification,
post-copy mutation, replacement, or digest mismatch exits 2 before nested Just
with zero repository-persistent write.

The exact R21 frozen-source order is eight paths:

1. `scripts/tasks/task-13/r21-shape.just`;
2. `scripts/tasks/task-13/guarded-just-r21.sh`;
3. `scripts/tasks/task-13/strict-entrypoint-r21.sh`;
4. `tests/nix/flake-shape-aarch64-linux-r21.sh`;
5. `scripts/tasks/task-13/flake-source-snapshot-r21.sh`;
6. `scripts/tasks/task-13/final-tree-record-r21.sh`;
7. `scripts/tasks/task-13/final-verify-r21.sh`;
8. `tests/nix/coverage-executor-toctou-r21.sh`.

`scripts/tasks/task-13/mod.just` is neither a frozen source nor an authoritative
R21 entrypoint. It remains byte-preserved during R21 authoring. Any existing or
later convenience wrapper is explicitly non-authoritative and cannot satisfy a
receipt, shape, flake, coverage, module, builder, final-verify, or FV acceptance
card.

## Digest-rooted strict bootstrap

A repository file cannot establish its own integrity against a persistent
pre-invocation attacker. The R21 trust anchors are therefore the lowercase
64-hex R21 manifest digest and strict-entrypoint digest separately returned to
and retained by the operator. Every authoritative R21 user card first copies
`strict-entrypoint-r21.sh` to a fresh temporary regular file, makes the copy
mode `0400`, verifies it against the operator-retained entrypoint digest, and
only then executes that copy. It never begins in `mod.just`.

The verified strict entrypoint has this exact ordered contract. Its only modes
are `receipt` and `pair <justfile> <private-recipe> [-- <recipe-args>]`:

1. require its operator-supplied entrypoint digest receipt, verify its own
   copied bytes against that digest, accept the operator-pinned R21 manifest
   digest, and reject malformed input;
2. resolve the live repository root, exact manifest, lock, record, and R21
   guard paths as regular non-symlink paths without escape;
3. copy the manifest to a fresh temporary regular file, make it mode `0400`,
   and verify canonical compact sorted JSON plus one LF and exact equality to
   the operator-pinned digest;
4. verify the live manifest still has that digest and the lock and coordinator
   record each equal it plus one LF;
5. extract the exact R21 guard path and digest only from the verified manifest
   copy, verify the live guard, copy it to a fresh temporary regular file, make
   it mode `0400`, and verify the copy against the manifest-bound digest;
6. pass only the physical live repository root, verified manifest-copy path,
   exact R21 generation, operator digest, requested allowlisted mode/pair, and
   arguments accepted by that pair's manifest-declared exact schema to the
   copied guard;
7. preserve the guard/nested exit and clean both temporary copies on success,
   failure, HUP, INT, or TERM.

The copied guard must require the strict-entrypoint receipts
`TASK13_R21_REPO_ROOT` and `TASK13_R21_MANIFEST_COPY`, reject an unverified
direct invocation, revalidate the live manifest/lock/record and frozen source
set, and then apply its normal receipt or dispatch protocol. The manifest owns
the exact strict-entrypoint path/hash, argument schema, trusted-host-tool
boundary, and per-card target selection. The literal operator card contains
the two retained digests and copies and verifies the entrypoint before any
repository script executes.

The five new private control pairs have closed argument schemas. Module
evaluation accepts exactly one of `s2-skeleton`, `s4-green`, or `s5-final`.
Each Linux or cross-system descriptor accepts exactly the GPE-map digest and
optionally the literal `fv07-replay`. Final verify accepts exactly one FV01
through FV08 dispatch ID followed by the GPE-map digest; FV09 has no shell
dispatch. Every other pair accepts zero recipe arguments. Extra, missing,
reordered, malformed, or unrecognized arguments fail before nested dispatch.

The protected boundary includes persistent and racing mutation of the live
manifest, guard, executor, and mutable wrapper after the operator pins the
receipt digests. The operator-held digests, literal copy/hash card, host shell,
`sha256sum`, `jq`, `cmp`, `stat`, `realpath`, and trusted Nix devShell are the
explicit external trust base. Compromise of that trust base is outside Task-13.

## Strict module, builder, and final/FV dispatch

No active R21 module, builder, final-verify, or FV card starts in `mod.just` or
by executing a helper directly. Each starts with the external entrypoint
copy/hash gate and selects the corresponding private recipe from frozen
`r21-shape.just`. The private recipes may call only a manifest-verified guard
pair or the two frozen R21 helpers. Direct helper command strings are internal
implementation steps recorded separately from the authoritative top-level
dispatch and cannot serve as user evidence.

`final-verify-r21-exec` accepts one FV01-FV08 selector and the exact GPE-map
digest. The final-verify inventory and catalog bind each FV ID to that single
strict top-level card, while separately binding its ordered internal steps.
Every active protocol producer, owner, and handoff lineage names the exact
versioned frozen helper path. Each catalog dispatch descriptor hashes both the
strict top-level command strings and its `internal_ordered_command_strings`;
an unversioned helper name or a direct helper command used as top-level
evidence is invalid.

For exactly FV01-FV08, the catalog, `dispatch_by_id`, and inventory must have
the same dispatch-ID set. Their three top-level command arrays and their three
internal command arrays must be byte-for-byte equal in JSON type, count,
order, and string value before PASS; observed equality without this explicit
gate is insufficient.

FV07's three replay steps select only the frozen `linux-builder-r21-exec`,
`aarch64-linux-builder-r21-exec`, and `cross-system-verify-r21-exec` pairs.
`final-tree-record-r21.sh` and `final-verify-r21.sh` are verified frozen helper
sources, never entrypoints. This preserves every FV identity, order, count,
descriptor, GPE, terminal-record, replay, handoff, and no-shell FV09 contract.

## Mandatory deterministic fixture coverage

The R21 executable fixture copies itself and every required R21 source into
each disposable repository and proves all of the following with exit 2, zero
unauthorized nested dispatch, and zero persistent write to the real repository:

- persistent pre-selection content mutation of `r21-shape.just`;
- persistent pre-selection mutation of `guarded-just-r21.sh`, rejected by the
  digest-rooted bootstrap before the copied guard runs;
- post-copy content mutation of the selected coverage executor source;
- post-copy path replacement of the selected coverage executor source; and
- malicious replacement of a mutable `mod.just` convenience wrapper, while
  strict bootstrap coverage succeeds without reading or invoking that wrapper
  and the wrapper sentinel remains zero.

The fixture must not use a weaker test-only validator. It invokes the same
operator copy/hash gate, frozen strict entrypoint, frozen guard, and production
validation primitives as the user card.

## Exact unchanged coverage contract

The R20 four-command array and LF-stream SHA remain byte-identical:
...
The canonical stream is each exact string followed by one LF, including the
final LF, with SHA-256
`bb77b0f773f339bc5f5767e57f96f6ad8352a299a53c1ff67f103595b4faa12f`.
No R21 change may alter it.

## Catalog and final-tree inheritance

The R21 current catalog requires exactly the five unique G0-F9 IDs:

- `g0-f9-authority-plan-snapshot`;
- `g0-f9-authority-requirements-snapshot`;
- `g0-f9-authority-approval`;
- `g0-f9-completion-token`;
- `g0-f9-token-integrity-review`.

The historical active rejection set is exactly 37 IDs: the two G0-F1
approved-plan/approved-requirements IDs plus each of the five authority plan,
requirements, approval, token, and token-review IDs for G0-F2 through G0-F8.
G0-F1 Task-4b prerequisite evidence remains separately classified and is not
rejected as that prerequisite. All final-tree, GPE, descriptor, receipt,
handoff, module, deployment, MinIO, timing, three-system, FV01-FV09, and
input/output-lineage contracts otherwise carry forward self-contained under
R21 names and `<r21-recorded-digest>`.

FV03's fifth authoritative card is the digest-rooted R21 bootstrap selecting
`scripts/tasks/task-13/r21-shape.just::coverage-r21-exec`. The prior
`scripts/tasks/task-13/mod.just coverage-r20 ...` card is historical blocked
evidence and no `mod.just coverage-r21 ...` command can replace the strict
card.

## Exact R21 completion-token schema

R21 token schema version is `1.9`. The complete schema is duplicated
byte-identically at `assertion_manifests.task13_r21.token_schema` and
`assertion_manifests.task13_r21.manifest_projection.token_schema`. It retains
canonical JSON plus one LF, exact root and normalized nested object key sets,
exact arrays/scalars/types, recursive instantiated-template equality, rejection
of unknown/missing/duplicate/null/unsubstituted content, and exactly ten
issuance placeholders for time, plan/requirements/approval hashes, and three
final plan-review paths/hashes. Schema and template digests are recomputed from
the final R21 candidate. The canonical schema object SHA-256 is
`950f54d1e991760bb136fac605be224236b0ad32603ae594f836ca60167b6c3a`;
its embedded placeholder-bearing canonical token template SHA-256 is
`d36e798dc5b216f1e3d999ae399767a9c3db0bd7600c84c48ff32469adb3f81f`.

The parent provenance records the exact R20 token, sole token review, issued
manifest/lock/record identity, alignment PASS review, safety FAIL review, and
confirmed defect listed above. These are issuance-only constants. The R21
token must not bind itself, its future post-token review, frozen sources,
fixture, strict bootstrap runtime output, R21 manifest/lock/record, CCR reviews,
receipt, shape/flake/coverage output, descriptor, FV, or handoff output.

## R21 candidate review history

The R21 R1 completeness review is immutable non-authoritative failure
evidence at
`.agents/results/review-task13-g0-f9-r21-plan-completeness-r1-20260907.md`
(`7934def478eaa91bdedbd976c1e8f594978c2628c8480cca3dec0b359ca97480`).
R2 replaced direct mutable `mod.just` and direct-helper final/FV entrypoints
with strict manifest-bound private pairs.

The R21 R2 completeness review is also immutable non-authoritative failure
evidence at
`.agents/results/review-task13-g0-f9-r21-plan-completeness-r2-20260907.md`
(`a466659ee8821f2632cc1cae04a309b2ce962ce364b4e411229ae87fb9094a35`).
R3 replaces every residual unversioned final-helper lineage and FV07
public-adapter wording with exact versioned frozen internal paths/pairs, and
binds each catalog descriptor's ordered internal commands.

The R21 R3 completeness review is likewise immutable non-authoritative
failure evidence at
`.agents/results/review-task13-g0-f9-r21-plan-completeness-r3-20260907.md`
(`66687bfaac30613a2c4209fbf9f6e844e788d89ee4aa06ba79e790bd854ecf5f`).
R4 makes the catalog/`dispatch_by_id`/inventory top-level and internal-array
equalities explicit acceptance invariants. It also assigns each of the three
FV07 replay receipts to its exact private R21 pair and to
`final-tree-record-r21.sh` as the internal-only frozen receipt owner after
strict top-level final-verify authentication.

R4 completeness passed at
`.agents/results/review-task13-g0-f9-r21-plan-completeness-r4-20260907.md`
(`a9394515360cfc7fc2d63161c6dbe57b00b4c07dfb6eef0936e972decb1e7a94`),
but that PASS is superseded and non-authoritative because the same candidate's
meta review failed at
`.agents/results/review-task13-g0-f9-r21-plan-meta-r4-20260907.md`
(`1eaad62cfd67fe7ca9faa2880bd490460a39c6ef2608a2df9c5eae01a2f44b10`).
The active pre-issuance rule still required an impossible R2 PASS set. R5
replaces that stale condition with the exact ordered R5 paths and requires
their recomputed file hashes and PASS statuses to bind identically through the
pre-token approval/token equality gate. No superseded review may satisfy an
approval or runtime gate; completeness, meta, and simplicity restart from the
exact R5 bytes.

## Mandatory R21 issuance order

1. Author the exact R21 candidate plan and these requirements, retaining the
   entire R20 plan/requirements bytes as historical prefix/context.
2. Run fresh exact-byte R21 completeness, meta, and simplicity reviews in that
   order. Any failure revises the candidate and restarts all three.
3. Before token issuance, prove the exact ordered review paths/hashes/PASS
   statuses are identical in plan, approval projection, and token projection,
   and prove every R21 generation surface equals the exact R21 generation.
4. Issue R21 approval and canonical schema-1.9 token.
5. Issue exactly one planned R21 post-token integrity PASS review.
6. Author the eight R21 frozen sources and implement the
   operator-digest-rooted strict entrypoint, the five additional private
   control pairs, the two frozen final helpers, and all five deterministic
   fixture modes.
7. Complete an independent pre-lock source audit with zero material finding.
8. Issue canonical R21 manifest, digest-plus-LF lock, and digest-plus-LF
   coordinator record once; never edit, relock, or rebaseline them.
9. Complete fresh R21 implementation alignment, safety, and simplicity CCR
   reviews in order. Any failure blocks all user cards and requires a new
   append-only successor.
10. Only after all CCR reviews PASS may the user run digest-rooted strict
    receipt, shape, guarded-flake-local, coverage, and later cards.

No Nix, Cargo, Just, build, test, format, Clippy, coverage, module/package,
Git, SCM, network, service, deployment, or production operation belongs to
R21 candidate planning or agent-owned implementation verification.
