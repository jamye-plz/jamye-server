...
use crate::ports::topics::{
    GetTopicQuery, ListTopicDatesQuery, ListTopicTagsQuery, ListTopicsQuery, TopicDatePage,
    TopicMediaRecord, TopicPage, TopicRecord, TopicStatus, TopicTagPage, TopicTagRecord,
    TopicTagSource, TopicsRepositoryError,
};
...
pub(super) async fn list_topic_dates(
    ...
    require_group_access(pool, query.group_id, query.actor_id).await?;
    let today = sqlx::query_scalar::<_, Date>(
        "SELECT timezone('Asia/Seoul', clock_timestamp())::date",
    )
    .fetch_one(pool)
    .await
