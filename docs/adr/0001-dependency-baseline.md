...
- 상태: Accepted, user lock/build evidence pending
- 날짜: 2026-08-24
...
`[features]`는 정확히 `default=[]`, `dev-fixtures=[]`로 시작한다. M0 default composition에는 fixture route/key/issuer가 없다. task-3c만 이 already-declared feature 뒤의 동작을 구현할 수 있다.

현재 direct version은 lock 생성 전 `Cargo.toml`의 official-current compatible requirement가 authoritative하다. lock card가 통과하면 이 문서에 별도 version literal을 복사하지 않고 `Cargo.lock` checksum으로 exact graph evidence를 남긴다. 이후 Cargo owner는 필요한 crate만 추가하고 같은 no-drift card를 다시 실행한다.
